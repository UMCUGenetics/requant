...
megalodon \
        ...
        --output-directory ./${file_base}/megalodon_sigmap/can_f \
        --outputs mappings signal_mappings \
        --num-reads 40000 \
        --guppy-server-path ../ont-guppy_v5/bin/guppy_basecall_server \
        --guppy-config dna_r9.4.1_450bps_fast.cfg \
...
mkdir -p ./${file_base}/taiyaki_merge_f/
python ../taiyaki/misc/merge_mappedsignalfiles.py \
        ./${file_base}/taiyaki_merge_f/merged_signal_mappings.hdf5 \
        --input ./${file_base}/megalodon_sigmap/can_f/signal_mappings.hdf5 None \
...
mkdir -p ./${file_base}/remora_prep_f/
remora \
        dataset prepare \
        ./${file_base}/taiyaki_merge_f/merged_signal_mappings.hdf5 \
        --output-remora-training-file ./${file_base}/remora_prep_f/remora_train_chunks.npz \
        --motif CG 0 \
        --chunk-context 50 50 \
        --kmer-context-bases 2 2 \
        --max-chunks-per-read 50 \
        --log-filename ./${file_base}/remora_prep_f/remora_prep_log.txt


# Actually train a model
# mkdir -p ./${file_base}/remora_train_f/
remora \
        model train ./${file_base}/remora_prep_f/remora_train_chunks.npz \
        ...
        --output-path ./${file_base}/remora_train_f/ \
        --device 0


# Call modified reads with trained model
mkdir -p ./${file_base}/megalodon_call_f/
megalodon \
        ${path_fast5_mod} \
        --outputs mod_mappings \
        --reference ${file_reference_full} \
        --remora-model ./${file_base}/remora_train_f/model_best.onnx \
        --write-mods-text \
        --processes ${num_threads} \
        --guppy-server-path ../ont-guppy_v5/bin/guppy_basecall_server \
        --guppy-params " \
        ...
        --devices cuda:0 \
        --output-directory ./${file_base}/megalodon_call_f/mod
...
megalodon \
        ...
        --guppy-config dna_r9.4.1_450bps_modbases_5mc_hac_prom.cfg \
        --devices cuda:0 \
        --output-directory ./${file_base}/megalodon_call_f/can